...
(function (global, factory) {
    ...
    // 定义一个空对象，且必能被修改，不能加属性，不能删属性不能修改原型
    var emptyObject = Object.freeze({});

    // These helpers produce better VM code in JS engines due to their
    // explicitness and function inlining.
    // 判断值为undefined或null
    // -设计模式-外观模式的一种应用，通过提供一个高级接口来完成判断，这样即使以后有更新了判断的方式，比如以后通过别的方式判断值是否为undefined，只需要更改接口，代码里N多使用的地方不用动
    function isUndef(v) {
        return v === undefined || v === null;
    }

    // 判断值为非undefined且不为null
    function isDef(v) {
        return v !== undefined && v !== null;
    }
     ...
     */
    // 判断值为object，要排除null值，因为null是一个特殊的object
     ...
     * is in that map.
     */
    // 通过,分割字符串，遍历key创建一个对象，返回一个函数判断参数是否在里面
    // var htmlStr = "body,div,ul,li..."
    // var isHtmlTag = makeMap(htmlStr)
    // var r = isHtmlTag("div")
     ...
     */
    // 从数组里移除一个item，匹配方式是通过indexOf查找的
     ...
     */
    // 高阶字符串处理函数，用来缓存已经处理过的字符串
    // 比如驼峰字符串处理，"abc-def"转换成"abcDef"，这样的过程，可以创建一个闭包，保存cache['abc-def'] = "abcDef"
    // 这样下次在碰到要转换"abc-def"可以直接从cache里取到结果"abcDef"
    // 核心原理就是变量读取过程的消耗要远远小于字符串的处理过程
    // 而vue中有大量的class名驼峰转换，这样一个高阶函数能提升很多性能
    // function cached(fn) {
    ...
    // bind调用判断，支持则用原生的，不支持则用polyfill，call和apply取决于参数个数
     ...
     */
    // reduce 数组累加器，对数组的每一项都作函数处理，函数需要返回一个值，该值会与total相加，可以在函数内部获取到当前值的index，和当前数组的状态
    // reduce提供了一个累加变量，相较于forEach这些方法，除了遍历数组，还能做其他的事
    // 例如一个list: [{a: true, n: 1}, {a: false, n: 2}, {a: true, n: 3}]，统计其中a为true的数量
    // 总是需要多声明一个变量
    // computed: {
    //   trueNum() {
    //     let t = 0;
    ...
    //   }
    // }
    // var list = [{a: true, n: 1}, {a: false, n: 2}, {a: true, n: 3}]
    // list.reduce((total, item) => {
    //     return item.a ? total + item.n : total
    // }, 0)
    // 4
    // arr.reduce(function(total, currentValue, currentIndex, arr){}, defaultValue)
    // 多很教程对函数的第一个参数都叫total，我更愿意理解为final，把数组的每一项的值处理完之后的结果给到final
    // 比如我们不做累加，只是单纯的把数组每一项的值返回，那么最终final会是数组最后一项的值
    // [2,3,4,5].reduce((final, curValue) => { return curValue })
    // 5
    // 对于复杂结构的数组，也可以将处理结果返回给final，初始值的结构要设置，否则会跟数组项结构一样
    // [{a: 1}, {a: 2}].reduce((final, curValue) => { return final.a + curValue.a}, 0)
    ...
    // 把modules里每一项的staticKeys取出来合成一个数组并用,连接
    ...
    var config = {
        ...
        // 通过Object.create(null)创建一个空对象，这个值与{}是不相等的，而且也不是Object的实例
