### my-vue
***
研究vue的核心原理，实现一个丐版的vue，双向绑定，template编译，发布订阅，观察者

![](https://raw.githubusercontent.com/heyach/blog/main/images/vuesourcecode/my-vue.gif)

### 实现思路
1. 完成data的单向绑定，即data发生变化可以监听到，从而去通知订阅者完成更新（后续步骤），利用Object.defineProperty
```js
...
2. 实现发布订阅，让某个值x订阅data的某个key，当key变化时，x能完成更新
```js
// 实现一个消息订阅器，发布订阅模式
function Dep() {
    this.subs = []
...
4. 实现template的解析，把html里的{{name}}添加到data里的订阅者里，当值改变时，完成html的更新
```js
var updater = {
...
6. 对data进行代理，实现`this.xxx == this.data.xxx`
```js
MVVM.prototype = {
    _proxy(key) {
